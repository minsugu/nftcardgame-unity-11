...
public class ListNFTWebGL : MonoBehaviour
{
    ...
    async void Start()
    {
        ...
        List<MintedNFT.Response> response =
            await EVM.GetMintedNFT(chain, network, account);

        if (response[0].uri.StartsWith("ipfs://"))
        {
            response[0].uri =
                response[0].uri.Replace("ipfs://", "https://ipfs.io/ipfs/");
        }
        ...
        RootGetNFT data =
            JsonConvert
                .DeserializeObject<RootGetNFT>(System
                    .Text
                    .Encoding
                    .UTF8
    ...
    IEnumerator DownloadImage(string MediaUrl)
    {
        ...
        else
        {
            Texture2D webTexture =
                ((DownloadHandlerTexture) request.downloadHandler).texture as
                Texture2D;
    ...
    public async void ListItem()
    {
        ...
        ListNFT.Response response =
            await EVM
                .CreateListNftTransaction(chain,
                network,
                account,
                _itemID,
                Convert.ToDecimal(wei).ToString(),
                _tokenType);
        ...
        try
        {
            string responseNft =
                await Web3GL
                    .SendTransactionData(response.tx.to,
                    value.ToString(),
